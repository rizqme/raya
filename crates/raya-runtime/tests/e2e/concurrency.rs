...
fn test_task_cancel() {
    expect_i32_with_builtins(
        "async function longRunning(): Promise<number> {
...
fn test_await_array_with_failure() {
    ...
    expect_i32(
        "async function succeed(): Promise<number> {
             return 42;
         }
...
fn test_chained_task_dependencies() {
    ...
    expect_i32(
        "async function taskA(): Promise<number> {
         ...
         async function taskC(): Promise<number> {
             let b = await taskB();
             return b + 3;
         }

         async function taskD(): Promise<number> {
             let c = await taskC();
             return c + 4;
         }
...
fn test_nested_parallel_await() {
    ...
    expect_i32(
        "async function leaf(x: number): Promise<number> {
         ...
         async function branch(x: number): Promise<number> {
             let t1 = leaf(x);
             let t2 = leaf(x + 1);
         ...
         async function root(): Promise<number> {
             let b1 = branch(1);
             let b2 = branch(10);
...
fn test_task_spawning_tasks() {
    ...
    expect_i32(
        "async function worker(depth: number): Promise<number> {
             if (depth <= 0) {
                 return 1;
             }
...
fn test_deep_async_call_stack() {
    ...
    expect_i32(
        "async function level5(): Promise<number> { return 5; }
         async function level4(): Promise<number> { return await level5() + 4; }
         async function level3(): Promise<number> { return await level4() + 3; }
         async function level2(): Promise<number> { return await level3() + 2; }
         async function level1(): Promise<number> { return await level2() + 1; }
         async function level0(): Promise<number> { return await level1() + 0; }
...
fn test_parallel_with_different_completion_times() {
    ...
    expect_i32(
        "async function quick(): Promise<number> {
             return 1;
         }

         async function medium(): Promise<number> {
             let x = 0;
             let i = 0;
             while (i < 10) {
                 x = x + 1;
...
fn test_rapid_spawn_and_complete() {
    ...
    expect_i32(
        "async function tiny(x: number): Promise<number> {
             return x;
         }
...
fn test_multiworker_task_chain() {
    ...
    expect_i32_multiworker(
        "async function taskA(): Promise<number> { return 1; }
         async function taskB(): Promise<number> { return await taskA() + 2; }
         async function taskC(): Promise<number> { return await taskB() + 3; }
         async function taskD(): Promise<number> { return await taskC() + 4; }
...
fn test_multiworker_work_stealing_stress() {
    ...
    expect_i32_multiworker(
        "async function heavy(x: number): Promise<number> {
             let sum = 0;
             let i = 0;
             while (i < x) {
                 sum = sum + 1;
