...
//!
//! Native implementation using the ABI for hashing, HMAC,
//! secure random, encoding, and constant-time comparison.

use raya_engine::vm::{
    buffer_allocate, buffer_read_bytes, string_allocate, string_read, NativeCallResult,
    NativeContext, NativeValue,
};
...
pub fn call_crypto_method(
    ...
    match method_id {
        ...
        0x400B => timing_safe_equal(args),   // TIMING_SAFE_EQUAL
...
fn hash_bytes(ctx: &NativeContext, args: &[NativeValue]) -> NativeCallResult {
    if args.len() < 2 {
    ...
    match hash_bytes_internal(&algorithm, &data) {
        Ok(digest) => NativeCallResult::Value(buffer_allocate(ctx, &digest)),
        Err(e) => NativeCallResult::Error(e),
    }
}

/// crypto.hmac(algorithm, key, data): string — hex-encoded MAC
fn hmac(ctx: &NativeContext, args: &[NativeValue]) -> NativeCallResult {
    if args.len() < 3 {
    ...
    let key = match string_read(args[1]) {
        Ok(s) => s,
        Err(e) => return NativeCallResult::Error(format!("Invalid key: {}", e)),
    };

    let data = match string_read(args[2]) {
        Ok(s) => s,
        Err(e) => return NativeCallResult::Error(format!("Invalid data: {}", e)),
    };
...
fn hmac_bytes(ctx: &NativeContext, args: &[NativeValue]) -> NativeCallResult {
    if args.len() < 3 {
    ...
    let key = match buffer_read_bytes(args[1]) {
        Ok(b) => b,
        Err(e) => return NativeCallResult::Error(format!("Invalid key: {}", e)),
    };

    let data = match buffer_read_bytes(args[2]) {
        Ok(b) => b,
        Err(e) => return NativeCallResult::Error(format!("Invalid data: {}", e)),
    };

    match hmac_compute_internal(&algorithm, &key, &data) {
        Ok(mac) => NativeCallResult::Value(buffer_allocate(ctx, &mac)),
        Err(e) => NativeCallResult::Error(e),
    }
}

/// crypto.randomBytes(size): Buffer
fn random_bytes(ctx: &NativeContext, args: &[NativeValue]) -> NativeCallResult {
    if args.is_empty() {
...
fn random_uuid(ctx: &NativeContext, _args: &[NativeValue]) -> NativeCallResult {
    let id = uuid::Uuid::new_v4().to_string();
    NativeCallResult::Value(string_allocate(ctx, id))
}

/// crypto.toHex(data: Buffer): string
fn to_hex(ctx: &NativeContext, args: &[NativeValue]) -> NativeCallResult {
    if args.is_empty() {
        return NativeCallResult::Error("crypto.toHex requires 1 argument".to_string());
    }

    let data = match buffer_read_bytes(args[0]) {
        Ok(b) => b,
        Err(e) => return NativeCallResult::Error(format!("Invalid buffer: {}", e)),
    };
...
fn from_hex(ctx: &NativeContext, args: &[NativeValue]) -> NativeCallResult {
    if args.is_empty() {
        return NativeCallResult::Error("crypto.fromHex requires 1 argument".to_string());
    }

    let hex_str = match string_read(args[0]) {
        Ok(s) => s,
...
fn to_base64(ctx: &NativeContext, args: &[NativeValue]) -> NativeCallResult {
    if args.is_empty() {
    ...
    NativeCallResult::Value(string_allocate(ctx, b64))
}

/// crypto.fromBase64(b64: string): Buffer
fn from_base64(ctx: &NativeContext, args: &[NativeValue]) -> NativeCallResult {
    if args.is_empty() {
        return NativeCallResult::Error("crypto.fromBase64 requires 1 argument".to_string());
    }

    let b64_str = match string_read(args[0]) {
        Ok(s) => s,
...
fn timing_safe_equal(args: &[NativeValue]) -> NativeCallResult {
    if args.len() < 2 {
        return NativeCallResult::Error(
    ...
    let a = match buffer_read_bytes(args[0]) {
        Ok(b) => b,
        Err(e) => return NativeCallResult::Error(format!("Invalid buffer a: {}", e)),
    };

    let b = match buffer_read_bytes(args[1]) {
        Ok(b) => b,
