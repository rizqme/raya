...
#[test]
fn fault_injection_result_shape_contract() {
    let workspace = workspace_root();
    let tmp_dir = unique_tmp_dir("fault-injection-shape");

    let out = run_cli_script(&workspace, &fault_injection_entry(), &tmp_dir);
    assert_ok_run(&out);

    let dir = tmp_dir.join("raya-examples-fault-injection");
    let summary = std::fs::read_to_string(dir.join("result.txt")).expect("fault result");
    let fields = parse_summary(&summary);
    assert_eq!(fields.len(), 5, "{summary}");
    for key in ["ok", "json", "hex", "missing", "panic"] {
        assert!(fields.contains_key(key), "missing field {key} in {summary}");
    }
