...
fn test_jsx_element_with_string_attr() {
    ...
    assert!(
        ir.contains("\"div\""),
        "IR should contain \"div\" tag:\n{}",
        ir
    );
    assert!(
        ir.contains("\"test\""),
        "IR should contain attr value \"test\":\n{}",
        ir
    );
    // Should produce an ObjectLiteral for the props
    assert!(
        ir.contains("object_literal"),
        "IR should contain ObjectLiteral for props:\n{}",
        ir
    );
}
...
fn test_jsx_boolean_attribute() {
    ...
    assert!(
        ir.contains("\"input\""),
        "IR should contain \"input\" tag:\n{}",
        ir
    );
    // Boolean attribute should produce true
    assert!(
        ir.contains("true"),
        "IR should contain true for boolean attr:\n{}",
        ir
    );
}
...
fn test_jsx_multiple_attributes() {
    ...
    assert!(
        ir.contains("\"1\""),
        "IR should contain attr value \"1\":\n{}",
        ir
    );
    assert!(
        ir.contains("\"2\""),
        "IR should contain attr value \"2\":\n{}",
        ir
    );
    assert!(
        ir.contains("\"3\""),
        "IR should contain attr value \"3\":\n{}",
        ir
    );
}
...
fn test_jsx_no_attributes_produces_null_props() {
    ...
    assert!(
        ir.contains("null"),
        "IR should contain null for empty props:\n{}",
        ir
    );
}
...
fn test_jsx_text_child() {
    ...
    assert!(
        ir.contains("\"Hello\""),
        "IR should contain text child \"Hello\":\n{}",
        ir
    );
}
...
fn test_jsx_nested_elements() {
    ...
    assert!(
        ir.contains("\"div\""),
        "IR should contain outer \"div\":\n{}",
        ir
    );
    assert!(
        ir.contains("\"span\""),
        "IR should contain inner \"span\":\n{}",
        ir
    );
    assert!(
        ir.contains("\"Hi\""),
        "IR should contain text \"Hi\":\n{}",
        ir
    );
}
...
fn test_jsx_multiple_children() {
    ...
    assert!(
        count_in(&ir, "\"span\"") >= 2,
        "IR should contain at least two \"span\" references:\n{}",
        ir
    );
}
...
fn test_jsx_empty_expression_child_skipped() {
    ...
    assert!(
        ir.contains("\"div\""),
        "IR should parse and contain \"div\":\n{}",
        ir
    );
}
...
fn test_jsx_component_uses_identifier() {
    ...
    assert!(
        ir.contains("call"),
        "IR should contain a call instruction:\n{}",
        ir
    );
}
...
fn test_jsx_fragment_multiple_children() {
    ...
    assert!(
        count_in(&ir, "\"span\"") >= 2,
        "IR should contain two span references:\n{}",
        ir
    );
}
...
fn test_jsx_namespaced_tag() {
    ...
    assert!(
        ir.contains("\"svg:path\""),
        "IR should contain \"svg:path\" string:\n{}",
        ir
    );
}
...
fn test_jsx_spread_attributes() {
    ...
    assert!(
        ir.contains("JSON.merge"),
        "IR should contain JSON.merge for spread:\n{}",
        ir
    );
}
...
fn test_jsx_disabled_produces_null() {
    ...
    assert!(
        ir.contains("null"),
        "IR should contain null when JSX disabled:\n{}",
        ir
    );
}
...
fn test_jsx_custom_factory() {
    ...
    assert!(
        ir.contains("call fn"),
        "IR should contain a direct call to the custom factory:\n{}",
        ir
    );
}
...
fn test_jsx_fragment_compiles_via_ir_pipeline() {
    ...
    assert!(
        result.is_ok(),
        "JSX fragment should compile via IR: {:?}",
        result.err()
    );
}
