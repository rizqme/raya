...
impl SharedVmState {
    ...
    /// Resolve a structural field slot only when the layout metadata exactly
    /// matches the object's physical field storage.
    pub fn structural_field_slot_index_for_object(
        &self,
        object: &crate::vm::object::Object,
        field_name: &str,
    ) -> Option<usize> {
    ...
    /// Resolve the field name for a physical structural slot only when the
    /// layout metadata exactly matches the object's physical field storage.
    pub fn structural_field_name_for_object_offset(
        &self,
        object: &crate::vm::object::Object,
        field_offset: usize,
    ) -> Option<String> {
    ...
    pub fn register_classes(&self, module: &Arc<Module>, nominal_type_base: usize) {
        ...
        for (i, class_def) in module.classes.iter().enumerate() {
            ...
            class.prototype_members = if class_def.runtime_instance_publication {
                Vec::new()
            } else {
                class_def
                    .methods
            ...
            class.runtime_instance_publication = class_def.runtime_instance_publication;
            class.static_members = if class_def.runtime_static_publication {
                Vec::new()
            } else {
                class_def
                    .static_methods
            ...
            class.runtime_static_publication = class_def.runtime_static_publication;
