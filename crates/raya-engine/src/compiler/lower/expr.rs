...
use super::{
    ClassFieldInfo, ConstantValue, Lowerer, ARRAY_TYPE_ID, BOOLEAN_TYPE_ID, CHANNEL_TYPE_ID,
    INT_TYPE_ID, JSON_OBJECT_TYPE_ID, JSON_TYPE_ID, MUTEX_TYPE_ID, NULL_TYPE_ID, NUMBER_TYPE_ID,
...
impl<'a> Lowerer<'a> {
    ...
    fn default_js_function_type(&self) -> TypeId {
    ...
    fn closure_value_type_for_expr(&self, expr: &Expression) -> TypeId {
    ...
    fn emit_member_spawn_call(
        ...
        let closure = self.lower_member_from_base(member, None);
        self.emit_closure_invoke(dest, closure, args, ClosureInvokeMode::Spawn);
    }
    ...
    fn emit_member_closure_invoke(
        ...
        args: Vec<Register>,
        call_ty: TypeId,
        completion: crate::semantics::CallCompletionKind,
    ) {
        if matches!(completion, crate::semantics::CallCompletionKind::Task) {
            self.emit_closure_invoke(dest, closure, args, ClosureInvokeMode::Spawn);
        } else {
            self.emit_js_member_call_helper(dest.clone(), receiver, closure, args);
    ...
    fn try_lower_static_identifier_member_call(
        ...
        if let Some(nominal_type_id) = self.static_member_owner_nominal_type(&member.object) {
            ...
            let receiver = if self.object_expr_is_class_constructor_ref(&member.object) {
        ...
        if let Some(crate::compiler::builtins::BuiltinSurfaceMemberDescriptor::Bound(binding)) =
            crate::compiler::builtins::BuiltinRegistry::shared()
                .type_descriptor(class_name)
                .and_then(|descriptor| descriptor.static_methods.get(method_name))
                .copied()
        {
    ...
    fn emit_builtin_kernel_call(
        ...
        let op = match op {
            ...
            .unwrap_or(KernelOp::VmNative(native_id)),
            other => other,
        };
    ...
    fn emit_builtin_namespace_member_call(
        ...
        if dispatch.is_get_own_property_descriptor_dispatch() {
    ...
    fn emit_member_call_from_dispatch(
        ...
        let Some(dispatch) = self
            ...
            let receiver = prelowered_object
                ...
                .unwrap_or_else(|| self.lower_expr(&member.object));
            let closure = self.lower_member_from_base(member, Some(receiver.clone()));
            let completion = if call_ty.as_u32() == TASK_TYPE_ID {
                crate::semantics::CallCompletionKind::Task
            } else {
                crate::semantics::CallCompletionKind::RuntimeClosure
            };
            self.emit_member_closure_invoke(
                dest.clone(),
                receiver,
        ...
        match dispatch.kind {
            crate::semantics::CallDispatchKind::StaticMethod => self
                .try_lower_static_identifier_member_call(
                    dest,
                    member,
                    method_name_symbol,
                    method_name,
            ...
            crate::semantics::CallDispatchKind::NominalMethod => {
                let nominal_type_id = self
                    .semantic_member_target(member.span.start, member.span.end)
                    .and_then(|target| target.receiver_type_id)
            ...
            crate::semantics::CallDispatchKind::StructuralCall => {
            ...
            crate::semantics::CallDispatchKind::ImportedNamespaceCall => {
                let receiver = prelowered_object
                    .clone()
                    .unwrap_or_else(|| self.lower_expr(&member.object));
                let closure = self.lower_member_from_base(member, Some(receiver.clone()));
            ...
            crate::semantics::CallDispatchKind::DynamicCall => {
                if let Some(nominal_type_id) = self.infer_nominal_type_id(&member.object) {
                    if let Some(result) = self.try_lower_nominal_member_call(
                        dest.clone(),
                        member,
            ...
            crate::semantics::CallDispatchKind::PlainFunction => None,
        }
    }
    ...
    fn emit_builtin_instance_property_read(
        ...
        match action {
            ...
            crate::compiler::type_registry::DispatchAction::Builtin(op) => {
            ...
            crate::compiler::type_registry::DispatchAction::DeclaredField {
                ...
                let member_ty = if inferred.as_u32() == UNRESOLVED_TYPE_ID {
                    field_type.map(TypeId::new).unwrap_or(UNRESOLVED)
                } else {
                    inferred
                };
    ...
    fn emit_structural_property_read(
        &mut self,
        member: &ast::MemberExpression,
        object: Register,
        property_name: &str,
    ) -> Option<Register> {
        if let Some((shape_id, field_index)) =
            self.structural_shape_slot_from_expr(&member.object, property_name)
        {
            let field_ty = self
                .structural_field_type_from_type(self.get_expr_type(&member.object), property_name)
                .unwrap_or_else(|| self.get_expr_type(&Expression::Member(member.clone())));
        ...
        let field_ty = self
            .structural_field_type_from_type(expr_ty, property_name)
            .unwrap_or_else(|| self.get_expr_type(&Expression::Member(member.clone())));
        let dest = self.alloc_register(field_ty);
        self.emit_structural_shape_name_registration_for_expr(&member.object);
        self.emit(IrInstr::LoadFieldShape {
    ...
    fn emit_property_read_from_dispatch(
        &mut self,
        member: &ast::MemberExpression,
        prelowered_object: Option<Register>,
    ) -> Option<Register> {
            ...
            self.errors
                .push(crate::compiler::CompileError::InternalError {
                    message: format!(
                        ...
                        self.checker_validated_receiver_type(&member.object)
                            .unwrap_or_else(|| self.get_expr_type(&member.object))
                            .as_u32(),
                        property_name
                    ),
                });
        ...
        match dispatch.kind {
            ...
            crate::semantics::PropertyDispatchKind::StructuralSlot => {
                self.emit_structural_property_read(member, object, property_name)
            }
            crate::semantics::PropertyDispatchKind::NominalField => None,
        }
    }

    fn emit_property_write_from_dispatch(
        &mut self,
        member: &ast::MemberExpression,
        object: Register,
        value: Register,
        ...
        match dispatch.kind {
            ...
            | crate::semantics::PropertyDispatchKind::DynamicProperty => {
                ...
                self.emit_js_named_property_assignment(object, property_name, value);
                true
            }
            crate::semantics::PropertyDispatchKind::StructuralSlot => {
                if let Some((shape_id, slot)) =
                    self.structural_shape_slot_from_expr(&member.object, property_name)
                {
                    ...
                    self.emit_member_store(&member.object, object, field_idx, property_name, value);
                    true
                } else {
                    self.emit_js_named_property_assignment(object, property_name, value);
                    true
            ...
            crate::semantics::PropertyDispatchKind::NominalField => false,
        }
    }
    ...
    fn try_lower_registry_member_dispatch(
        ...
        match action {
            crate::compiler::type_registry::DispatchAction::Builtin(mut op) => {
    ...
    fn member_write_uses_dynamic_property_path(
        ...
        matches!(
            self.type_ctx.get(self.get_expr_type(object_expr)),
            Some(Type::Function(_))
        ) || matches!(self.type_ctx.get(object_reg_ty), Some(Type::Function(_)))
    }

    fn member_read_uses_dynamic_property_path(
        &self,
        object_expr: &Expression,
        object_reg_ty: TypeId,
    ) -> bool {
        matches!(
            ...
            self.type_ctx.get(object_reg_ty),
            Some(Type::Function(_))
                | Some(Type::Array(_))
                | Some(Type::Tuple(_))
                | Some(Type::Primitive(PrimitiveType::String))
    ...
    fn object_expr_is_class_constructor_ref(&self, expr: &Expression) -> bool {
        matches!(
            self.semantic_object_shape_kind(expr.span().start, expr.span().end),
    ...
    pub(super) fn type_is_handle_dispatch_builtin(&self, ty: TypeId) -> bool {
    ...
    fn effective_identifier_value_type(&self, ident: &ast::Identifier, fallback: TypeId) -> TypeId {
        ...
        if fallback.as_u32() != UNRESOLVED_TYPE_ID
            && fallback != narrowed
            && self.type_is_handle_dispatch_builtin(fallback)
    ...
    fn projected_structural_layout_from_expr(
        ...
        match object_expr {
            Expression::Identifier(ident) => {
                ...
                ) || self.binding_uses_runtime_dispatch_hint(ident.name)
                {
                    return None;
                }
    ...
    fn propagate_variable_projection_to_register(
        ...
        if let Some(fields) = self
            .runtime_constructor_type_cache
            .get(&var_name)
    ...
    fn emit_load_js_script_lexical_global(
        ...
        if let Some(&init_idx) = self.js_script_lexical_init_globals.get(&ident.name) {
            ...
            if !self.binding_uses_runtime_dispatch_hint(ident.name) {
    ...
    fn emit_store_js_script_lexical_global(&mut self, symbol: Symbol, value: Register) -> bool {
        ...
        self.current_function_mut()
            .add_block(BasicBlock::with_label(
                exit_block,
                "script.lexical.store.exit",
            ));
        self.current_block = exit_block;
        true
    }

    pub(super) fn env_handle_for_binding(
        &self,
        direct_eval: bool,
        global: bool,
        object_with: bool,
    ...
    fn semantic_identifier_resolution_kind(
        &self,
        span_start: usize,
    ) -> Option<crate::semantics::ResolvedIdentifierKind> {
        self.semantic_plan
            .resolved_identifier_at_span(span_start)
            .map(|resolved| resolved.kind)
    }

    fn semantic_identifier_requires_runtime_lookup(&self, span_start: usize) -> bool {
        matches!(
            self.semantic_identifier_resolution_kind(span_start),
    ...
    fn semantic_identifier_is_builtin_global(&self, span_start: usize) -> bool {
        matches!(
            self.semantic_identifier_resolution_kind(span_start),
    ...
    fn semantic_call_target(
        ...
        self.semantic_plan.call_target_at_span(span_start, span_end)
    }

    fn semantic_property_dispatch(
        &self,
        span_start: usize,
        span_end: usize,
        ...
        match builtin_dispatch.effective_builtin_op_id() {
            ...
            Some(op) => {
                let args = new_expr
                    ...
                    .map(|arg| self.lower_expr(arg.expression()))
                    .collect::<Vec<_>>();
                self.emit_builtin_kernel_call(Some(dest.clone()), KernelOp::Builtin(op), args);
                Some(dest)
            }
            _ => None,
        }
    ...
    fn semantic_value_origin_kind_for_expr(
        &self,
        expr: &Expression,
    ) -> Option<crate::semantics::ValueOriginKind> {
        self.semantic_value_origin(expr.span().start, expr.span().end)
            .map(|origin| origin.kind)
    }

    fn name_is_non_shadowable_intrinsic_global(&self, name: &str) -> bool {
        matches!(name, "Infinity" | "NaN" | "undefined" | "globalThis")
    }

    pub(super) fn identifier_constructor_binding_hint(
        &self,
        ident: &ast::Identifier,
        ...
        match origin {
            crate::semantics::ValueOriginKind::BoundValue
            | crate::semantics::ValueOriginKind::ImportedBinding
            | crate::semantics::ValueOriginKind::BuiltinGlobalValue
            | crate::semantics::ValueOriginKind::BuiltinNamespace
            | crate::semantics::ValueOriginKind::RuntimeLateBoundValue => {
                Some((ident.name, constructor_type))
            }
            crate::semantics::ValueOriginKind::ImportedNamespace
            | crate::semantics::ValueOriginKind::RuntimeEnvValue
            | crate::semantics::ValueOriginKind::DynamicValue => None,
        }
    }

    pub(super) fn new_expr_runtime_dispatch_binding_hint(
        &self,
        new_expr: &ast::NewExpression,
    ) -> Option<(Symbol, Option<TypeId>)> {
        let dispatch =
            self.semantic_constructor_dispatch(new_expr.span.start, new_expr.span.end)?;
        ...
        match dispatch.kind {
            ...
            | crate::semantics::ConstructorDispatchKind::RuntimeConstructorValue => {
                Some((ident.name, ctor_ty))
            }
            crate::semantics::ConstructorDispatchKind::NominalClass
            | crate::semantics::ConstructorDispatchKind::BuiltinNativeConstructor
            | crate::semantics::ConstructorDispatchKind::DynamicConstructor => None,
        }
    }
    ...
    fn semantic_object_shape_kind(
        ...
        self.semantic_object_shape(span_start, span_end)
            .map(|shape| shape.kind)
    }

    fn semantic_object_shape_type_id(&self, span_start: usize, span_end: usize) -> Option<TypeId> {
        self.semantic_object_shape(span_start, span_end)
            .and_then(|shape| shape.type_id)
    }

    fn checker_validated_receiver_type(&self, expr: &Expression) -> Option<TypeId> {
        let expr_ty = self.get_expr_type(expr);
        if expr_ty.as_u32() != UNRESOLVED_TYPE_ID {
            return Some(expr_ty);
        }
        self.semantic_object_shape_type_id(expr.span().start, expr.span().end)
    }

    fn receiver_has_checker_validated_members(&self, expr: &Expression) -> bool {
        self.checker_validated_receiver_type(expr)
            .is_some_and(|ty| self.type_has_checker_validated_class_members(ty))
    }

    fn semantic_call_completion_kind(
        &self,
        span_start: usize,
        span_end: usize,
    ) -> crate::semantics::CallCompletionKind {
        self.semantic_call_dispatch(span_start, span_end)
            .map(|dispatch| dispatch.completion_kind)
    ...
    fn semantic_identifier_is_delete_localish(&self, ident: &ast::Identifier, name: &str) -> bool {
        match self.semantic_identifier_resolution_kind(ident.span.start) {
            ...
            | Some(crate::semantics::ResolvedIdentifierKind::BuiltinGlobal) => true,
            Some(crate::semantics::ResolvedIdentifierKind::RuntimeEnvLookup)
            | Some(crate::semantics::ResolvedIdentifierKind::AmbientGlobal) => false,
            None => {
    ...
    pub(super) fn resolve_identifier_binding(
        &mut self,
        symbol: Symbol,
        span_start: usize,
                ...
                && self.semantic_identifier_requires_runtime_lookup(span_start))
        {
            return Some(super::ResolvedBinding::DirectEval {
                env: self.env_handle_for_binding(true, false, false),
                symbol,
            });
        }
        ...
        if self.active_runtime_declaration_bindings.contains(name) {
            return Some(super::ResolvedBinding::RuntimeIdentifier {
                env: self.env_handle_for_binding(false, false, false),
                symbol,
            });
        ...
        match binding {
            super::ResolvedBinding::Local {
                ...
                if is_refcell {
                    ...
                    if !self.binding_uses_runtime_dispatch_hint(symbol) {
            ...
            super::ResolvedBinding::RuntimeIdentifier { symbol, .. } => {
    ...
    pub(super) fn emit_store_identifier_binding(
        &mut self,
        binding: super::ResolvedBinding,
        value: Register,
    ) -> Register {
    ...
    fn static_member_owner_nominal_type(&self, object_expr: &Expression) -> Option<NominalTypeId> {
        match object_expr {
            ...
            Expression::Identifier(ident) => self
                ...
                .or_else(|| self.unshadowed_class_identifier_nominal_type(ident)),
            Expression::This(_)
    ...
    fn unshadowed_class_identifier_nominal_type(
        &self,
        ident: &ast::Identifier,
    ) -> Option<NominalTypeId> {
        if self.local_map.contains_key(&ident.name)
            || self.named_function_self_bindings.contains_key(&ident.name)
    ...
    pub(super) fn emit_direct_eval_binding_get(
        &mut self,
        name: &str,
        non_throwing: bool,
    ) -> Register {
    ...
    fn semantic_scope_binding(
        ...
        self.semantic_scope_snapshot(span_start, span_end)?
            .iter()
            .copied()
            .find(|binding| binding.symbol == symbol)
    }

    fn direct_eval_scope_bindings(
        &self,
        span_start: usize,
        span_end: usize,
    ) -> Vec<crate::semantics::ScopeSnapshotBinding> {
        let mut bindings = self
            .semantic_scope_snapshot(span_start, span_end)
            .map(|snapshot| snapshot.to_vec())
            .unwrap_or_default();
        let mut seen: FxHashSet<Symbol> = bindings.iter().map(|binding| binding.symbol).collect();
        if let Some(method_bindings) = &self.current_method_env_globals {
            for &symbol in method_bindings.keys() {
                if seen.insert(symbol) {
                    bindings.push(crate::semantics::ScopeSnapshotBinding {
                        symbol,
                        kind: crate::BindingKind::Var,
                        top_level: false,
                        runtime_env: false,
                        in_tdz: false,
                    });
    ...
    fn direct_eval_binding_is_lexical_at_span(
        &self,
        span_start: usize,
        span_end: usize,
        symbol: Symbol,
    ) -> bool {
        self.semantic_scope_binding(span_start, span_end, symbol)
            .is_some_and(|binding| {
                matches!(
                    binding.kind,
                    crate::BindingKind::Lexical
                        | crate::BindingKind::Class
                        | crate::BindingKind::Parameter
                )
            })
    }
    ...
    pub(super) fn lower_direct_eval_environment_object(
        ...
        for binding in self.direct_eval_scope_bindings(span_start, span_end) {
            ...
            let name = self.interner.resolve(symbol).to_string();
            let local_idx = self.local_map.get(&symbol).copied();
            let is_parameter_binding = binding.kind == crate::BindingKind::Parameter;
            let is_uninitialized_parameter =
                is_parameter_binding && self.parameter_tdz_symbols.contains(&symbol);
            let is_uninitialized_local = binding.in_tdz && !is_parameter_binding;
    ...
    pub(super) fn lower_activation_direct_eval_environment_object(
        &mut self,
        span_start: usize,
        span_end: usize,
    ) -> Register {
        ...
        for symbol in locals {
            ...
            let local_idx = self
                ...
                .get(&symbol)
                .copied()
                .expect("activation eval env local must exist");
            let semantic_binding = self.semantic_scope_binding(span_start, span_end, symbol);
            let is_parameter_binding = semantic_binding
    ...
    pub fn lower_expr(&mut self, expr: &Expression) -> Register {
        ...
        match expr {
            ...
            Expression::Function(func) => self.lower_function_expression(func),
    ...
    fn lower_regex_literal(&mut self, regex: &ast::RegexLiteral) -> Register {
        ...
        // Go through dynamic class construction so literals produce full RegExp
    ...
    pub(super) fn emit_constant_value(&mut self, const_val: &ConstantValue) -> Register {
        match const_val {
            ConstantValue::I64(v) => {
                ...
                let dest = self.alloc_register(ty);
                let value = if self.js_this_binding_compat {
                    IrValue::Constant(IrConstant::F64(*v as f64))
                } else {
                    IrValue::Constant(IrConstant::I32(*v as i32))
                };
                self.emit(IrInstr::Assign {
                    dest: dest.clone(),
                    value,
    ...
    pub(super) fn lower_identifier(&mut self, ident: &ast::Identifier) -> Register {
        ...
        if let Some(current_class) = self.current_class {
            let is_unshadowed_current_class_name =
                self.nominal_type_id_from_type_name(name) == Some(current_class)
                    ...
                    && !self
                        ...
                        .as_ref()
                        .is_some_and(|ancestors| ancestors.contains_key(&ident.name));
            if is_unshadowed_current_class_name {
                ...
                return self.load_class_value_for_nominal_type(current_class);
            }
        }
            ...
            if self.js_dynamic_semantics_enabled() && self.js_this_binding_compat {
                return self.lower_eval_shadowable_identifier(name, |this| {
                    if is_refcell {
                        ...
                        if !this.binding_uses_runtime_dispatch_hint(ident.name) {
        ...
        // Ambient builtin globals are resolved at runtime through a dedicated
        // lookup path and do not require source-level declarations.
    ...
    pub(super) fn load_class_value_for_nominal_type(
        ...
        if let Some(class_symbol) = self
            .class_map
            .iter()
            .find_map(|(&symbol, &id)| (id == nominal_type_id).then_some(symbol))
        {
            if let Some(&global_idx) = self.module_var_globals.get(&class_symbol) {
            ...
            if self.js_this_binding_compat {
                if let Some(&global_idx) = self.js_script_lexical_globals.get(&class_symbol) {
                    let ident = ast::Identifier {
                        name: class_symbol,
        ...
        self.load_raw_class_value_for_nominal_type(nominal_type_id)
    }

    pub(super) fn load_raw_class_value_for_nominal_type(
        &mut self,
        nominal_type_id: NominalTypeId,
    ) -> Register {
        let dest = self.alloc_register(self.default_js_function_type());
    ...
    fn lower_unary(&mut self, unary: &ast::UnaryExpression) -> Register {
        ...
        match unary.operator {
            ...
            // Strict Raya preserves the historical `null` result for `void`.
            // Node-compat keeps JavaScript's `undefined`.
            ast::UnaryOperator::Void => {
                let _operand = self.lower_expr(&unary.operand);
                let dest = self.alloc_register(TypeId::new(UNKNOWN_TYPE_ID));
                self.emit(IrInstr::Assign {
                    dest: dest.clone(),
                    value: if self.js_this_binding_compat {
                        IrValue::Constant(IrConstant::Undefined)
                    } else {
    ...
    fn lower_delete(&mut self, unary: &ast::UnaryExpression) -> Register {
        ...
        if let Expression::Identifier(ident) = &*unary.operand {
            let name = self.interner.resolve(ident.name);
            let resolved_locally = self.semantic_identifier_is_delete_localish(ident, name);
            if self.js_this_binding_compat {
    ...
    fn lower_increment_decrement(&mut self, unary: &ast::UnaryExpression) -> Register {
        ...
        if let Expression::Identifier(ident) = &*unary.operand {
            self.store_identifier_value_at_span(ident.name, ident.span.start, new_value.clone());
        }
    ...
    fn lower_call(&mut self, call: &ast::CallExpression, full_expr: &Expression) -> Register {
        ...
        let semantic_call_kind = self
            ...
            .map(|op| op.kind);
        let semantic_call_target = self
            .semantic_call_target(call.span.start, call.span.end)
            .cloned();
        ...
        if self.js_this_binding_compat {
            if let Expression::Member(member) = &*call.callee {
                ...
                if helper_name == "apply" && target_may_be_callable && !call.has_spread_arguments()
                {
                    ...
                    self.emit_js_apply_helper(dest.clone(), receiver, target_callable, args_array);
                    self.propagate_type_projection_to_register(dest.ty, &dest);
                    return dest;
                }
            }
        ...
        if let Expression::Super(_) = &*call.callee {
            if let Some(current_nominal_type_id) = self.current_class {
                ...
                if let Some(parent_constructor) = parent_constructor {
                    ...
                    // `super(...)` hops instead of reallocating a parent-shaped object
                ...
                if bind_name == "bind" {
                    if let Expression::Member(target_member) = &*bind_member.object {
                        if let Some(nominal_type_id) =
                            self.infer_nominal_type_id(&target_member.object)
                        {
        ...
        if let Expression::Identifier(ident) = &*call.callee {
            ...
            let identifier_resolved_locally = if self.js_this_binding_compat {
                matches!(
                    self.semantic_identifier_resolution_kind(ident.span.start),
                    Some(crate::semantics::ResolvedIdentifierKind::LocalBinding)
                        | Some(crate::semantics::ResolvedIdentifierKind::CaptureBinding)
                        | Some(crate::semantics::ResolvedIdentifierKind::ScriptGlobalBinding)
                )
            ...
            if self.js_this_binding_compat && is_direct_eval_call && !identifier_resolved_locally {
                ...
                let mut env_object =
                    self.lower_direct_eval_environment_object(call.span.start, call.span.end);
            ...
            if self.js_this_binding_compat && Self::js_builtin_call_uses_construct(name) {
                ...
                return self.lower_new(&synthetic_new);
            }
            ...
            if name == "String" && !self.js_this_binding_compat {
                if let Some(value) = args.first().cloned() {
        ...
        if let Expression::Member(member) = &*call.callee {
            let method_name = self.interner.resolve(member.property.name);
            let prelowered_member_object = match member.object.as_ref() {
                Expression::Identifier(_) | Expression::This(_) | Expression::Super(_) => None,
            ...
            self.errors
                .push(crate::compiler::CompileError::InternalError {
                    message: format!(
                        "semantic member dispatch '{}(...)' did not emit IR",
                        method_name
                    ),
                });
            self.poison_register(&dest);
        ...
        let ambient_runtime_callable = matches!(
            self.semantic_value_origin_kind_for_expr(&call.callee),
            Some(
                crate::semantics::ValueOriginKind::BuiltinGlobalValue
                    | crate::semantics::ValueOriginKind::BuiltinNamespace
            )
    ...
    fn lower_member(&mut self, member: &ast::MemberExpression) -> Register {
        self.lower_member_from_base(member, None)
    }

    fn nominal_member_call_requires_property_get(
        &self,
        nominal_type_id: NominalTypeId,
        method_name_symbol: Symbol,
    ) -> bool {
        ...
        while let Some(class_id) = current {
            ...
            if class_info.methods.iter().rev().any(|method| {
                method.name == method_name_symbol && method.kind == ast::MethodKind::Getter
            }) {
                return true;
            }
    ...
    fn lower_member_from_base(
        ...
        if !early_nominal_field_access
            ...
            Some(
                crate::semantics::MemberTargetKind::NominalMethod
                    | crate::semantics::MemberTargetKind::StaticMethod
            )
        ) {
            if let Some(result) =
                self.emit_property_read_from_dispatch(member, prelowered_object.clone())
            {
                return result;
            }
        }
        ...
        if let Some(nominal_type_id) = self.static_member_owner_nominal_type(&member.object) {
            ...
            if let Some(&func_id) = self
                .static_method_map
                .get(&(nominal_type_id, member.property.name))
            {
                let member_ty = {
                    let member_expr = Expression::Member(member.clone());
                    let inferred = self.get_expr_type(&member_expr);
        ...
        if let Some(nominal_type_id) = nominal_type_id {
            let getter_func_id = {
                ...
                while let Some(class_id) = current {
                    ...
                    if let Some(method_info) = class_info.methods.iter().rev().find(|method| {
                        method.name == member.property.name
                            && method.kind == ast::MethodKind::Getter
                    }) {
        ...
        let receiver_is_function_object =
            matches!(self.type_ctx.get(receiver_expr_ty), Some(Type::Function(_)))
                || matches!(self.type_ctx.get(object.ty), Some(Type::Function(_)));
        let receiver_is_dynamic_object =
            matches!(
                ...
                || receiver_is_function_object;
        let receiver_has_structural_named_view = nominal_type_id.is_none()
            && (self
                .structural_shape_slot_from_expr(&member.object, prop_name)
                .is_some()
                || self
        ...
        let resolved_field = if let Some(nominal_type_id) = nominal_type_id {
            ...
            let infer_field_from_expr_type = |this: &Self| {
                ...
                {
                    let field_ty = this
                        .structural_field_type_from_type(
            ...
            let alias_field_slot = match &*member.object {
                Expression::Identifier(ident) => self
                    .variable_object_type_aliases
                    .get(&ident.name)
                    .and_then(|alias| {
                        self.type_alias_field_lookup(alias, prop_name).and_then(
                            |(field_index, field_ty)| {
                                let alias_shape_id = self
                                    .projected_structural_layout_from_alias_name(alias)
                                    .map(|layout| {
                                        let names = layout
                                            .into_iter()
                                    ...
                                    .or_else(|| {
                                        self.structural_shape_id_from_type(
                                            self.get_expr_type(&member.object),
                                        )
                                    })?;
                                Some((field_index, field_ty, alias_shape_id))
                            },
                        )
                    }),
                _ => None,
            ...
            if let Some((field_index, field_ty, shape_id)) = alias_field_slot {
                Some(ResolvedMemberSlot::Shape {
                    shape_id,
                    field_index,
                    field_ty,
    ...
    fn lower_index(&mut self, index: &ast::IndexExpression, full_expr: &Expression) -> Register {
        ...
        let uses_dynamic_keyed_access = self.index_uses_dynamic_keyed_access_for_expr(
            self.get_expr_type(&index.object),
            &index.index,
        );
    ...
    fn expr_is_es_array_index_key(&self, expr: &Expression) -> bool {
        match expr {
            ...
            Expression::StringLiteral(lit) => {
                let key = self.interner.resolve(lit.value);
                if key.is_empty() {
                    return false;
                }
    ...
    fn expr_is_sequence_element_index_expr(&self, expr: &Expression) -> bool {
        match expr {
            ...
            _ => self.expr_is_es_array_index_key(expr),
        }
    }

    fn index_uses_dynamic_keyed_access_for_expr(
        &self,
        object_ty: TypeId,
        ...
        match self.type_ctx.get(object_ty) {
            Some(Type::Array(_)) | Some(Type::Tuple(_)) => {
                !self.expr_is_sequence_element_index_expr(index_expr)
            }
            Some(Type::Primitive(PrimitiveType::String)) => {
                !self.expr_is_es_array_index_key(index_expr)
            }
            _ => false,
        }
    }
    ...
    fn lower_object_property_key(&mut self, key: &ast::PropertyKey) -> Register {
        match key {
            ast::PropertyKey::Identifier(ident) => {
                self.emit_named_key_register(self.interner.resolve(ident.name))
            }
    ...
    pub(super) fn lower_accessor_property_descriptor(
        ...
        if let Some(setter) = setter {
            self.emit_dyn_set_named(descriptor.clone(), "set", setter);
        }

        let enumerable_reg = self.alloc_register(TypeId::new(BOOLEAN_TYPE_ID));
        self.emit(IrInstr::Assign {
            dest: enumerable_reg.clone(),
            value: IrValue::Constant(IrConstant::Boolean(enumerable)),
        });
        self.emit_dyn_set_named(descriptor.clone(), "enumerable", enumerable_reg);

        let configurable_reg = self.alloc_register(TypeId::new(BOOLEAN_TYPE_ID));
        self.emit(IrInstr::Assign {
            dest: configurable_reg.clone(),
            value: IrValue::Constant(IrConstant::Boolean(configurable)),
        });
        self.emit_dyn_set_named(descriptor.clone(), "configurable", configurable_reg);
    ...
    pub(super) fn resolve_concrete_class_type_for_runtime_slots(
        ...
        match ty {
            crate::parser::types::Type::Class(class_ty) => self
                .nominal_type_id_from_type_id(ty_id)
                .or_else(|| self.nominal_type_id_from_type_name(&class_ty.name))
                .map(|_| ty_id),
            crate::parser::types::Type::Reference(reference) => self
                .type_ctx
                .lookup_named_type(&reference.name)
    ...
    fn attach_home_object_if_method(
        &mut self,
        value_expr: &ast::Expression,
        value: &Register,
        home_object: &Register,
    ) {
        if matches!(
            value_expr,
            ...
            match prop {
                ast::ObjectProperty::Property(p) => {
                    if p.kind != ast::PropertyKind::Init
                        || matches!(p.key, ast::PropertyKey::Computed(_))
                    {
                        continue;
                    }
    ...
    pub(super) fn store_identifier_value_at_span(
        &mut self,
        symbol: Symbol,
        span_start: usize,
        value: Register,
    ) {
        if self.constant_map.contains_key(&symbol) || self.immutable_bindings.contains(&symbol) {
            let _ = self.emit_type_error_throw("Assignment to constant variable.");
            return;
        }
        if self.js_this_binding_compat && self.semantic_identifier_is_in_tdz(span_start) {
    ...
    fn identifier_has_static_assignment_target(&self, symbol: Symbol) -> bool {
        self.local_map.contains_key(&symbol)
            ...
            || self
                ...
                .contains(self.interner.resolve(symbol))
    ...
    fn emit_iterator_throw_guard<F>(&mut self, iterator: Register, step_result: Register, body: F)
    where
        F: FnOnce(&mut Self),
    {
        let catch_block = self.alloc_block();
                ...
                for elem_opt in &array_pat.elements {
                    ...
                    match elem {
                        ast::ArrayElement::Expression(expr) => {
                            ...
                            let elem_reg = self.emit_iterator_value_helper(step_result.clone());
                            self.emit(IrInstr::Assign {
                                dest: final_val.clone(),
                                ...
                                |this| {
                                    if let Expression::Assignment(assign) = expr {
                                        if assign.operator == AssignmentOperator::Assign {
                                            let final_val = this
                                                .emit_destructuring_assignment_default_value(
                                                    &assign.left,
                                                    final_val.clone(),
                                                    &assign.right,
                                                );
                                    ...
                                    if let Some(prepared_target) = prepared_target.clone() {
                                        ...
                                        this.lower_destructuring_assignment_target(expr, final_val);
                                    }
                                },
                            );
                        }
                        ast::ArrayElement::Spread(expr) => {
                            ...
                            if let Some(prepared_target) = prepared_target {
                                self.store_prepared_destructuring_target(prepared_target, rest_arr);
                            } else {
    ...
    fn lower_assignment(&mut self, assign: &ast::AssignmentExpression) -> Register {
        ...
        if matches!(
            ...
            match &*assign.left {
                ...
                Expression::Member(member) => {
                    ...
                    if self.emit_property_write_from_dispatch(member, object.clone(), rhs.clone()) {
                    } else if use_js_runtime_member_semantics {
                            ...
                            .find(|f| self.interner.resolve(f.name) == prop_name)
                        {
                            ...
                            if allow_dynamic_any_write {
                                ...
                                let class_name = self
                                    ...
                                    .unwrap_or_else(|| {
                                        format!("class#{}", nominal_type_id.as_u32())
                                    });
                                self.errors
                                    .push(crate::compiler::CompileError::InternalError {
                                        message: format!(
            ...
            let result = self.alloc_register(current.ty);
            self.emit(IrInstr::Phi {
                dest: result.clone(),
                sources: vec![(entry_block, current), (assign_block, assigned_value)],
            });
            return result;
        }
        ...
        let binary_op = match assign.operator {
            ...
            AssignmentOperator::LogicalOrAssign | AssignmentOperator::LogicalAndAssign => {
                unreachable!()
            }
        ...
        match &*assign.left {
            Expression::Array(array_lhs) => {
                if assign.operator == AssignmentOperator::Assign {
                    for (idx, elem) in array_lhs.elements.iter().enumerate() {
                        let Some(ast::ArrayElement::Expression(target_expr)) = elem else {
                            continue;
                        };
                        let Expression::Identifier(ident) = target_expr else {
                            continue;
                        };

                        let idx_reg = self.alloc_register(TypeId::new(NUMBER_TYPE_ID));
                        self.emit(IrInstr::Assign {
                            dest: idx_reg.clone(),
                            value: IrValue::Constant(IrConstant::I32(idx as i32)),
                        });
                        let elem_reg = self.alloc_register(value.ty);
                        self.emit(IrInstr::LoadElement {
                            dest: elem_reg.clone(),
                            array: value.clone(),
                            index: idx_reg,
                        });
                        self.store_identifier_value_at_span(ident.name, ident.span.start, elem_reg);
                    }
                }
            }
            Expression::Identifier(ident) => {
            ...
            Expression::Member(member) => {
                ...
                if self.emit_property_write_from_dispatch(member, object.clone(), value.clone()) {
                } else if use_js_runtime_member_semantics {
    ...
    pub(super) fn lower_function_expression_with_preassigned_id(
        ...
        if let Some(name) = &func.name {
            ...
            named_self_local = Some(local_idx);
        }
                ...
                .take_while(|stmt| {
                    matches!(
                        ...
                        ast::Statement::Expression(ast::ExpressionStatement {
                            expression: ast::Expression::StringLiteral(_),
                            ..
                        })
                    )
                })
        ...
        let lowered_function = self.current_function.take().unwrap();
        ...
        let this_reg_for_closure = if child_this_captured_idx.is_some() {
            let this_reg = self.alloc_register(TypeId::new(NUMBER_TYPE_ID));
            if self.this_register.is_some() {
                self.emit(IrInstr::LoadLocal {
                    dest: this_reg.clone(),
                    index: 0,
        ...
        for cap in &captured_vars {
            let cap_reg = self.alloc_register(cap.ty);
            match cap.source {
                super::AncestorSource::ImmediateParentLocal(local_idx) => {
                    self.emit(IrInstr::LoadLocal {
                        dest: cap_reg.clone(),
        ...
        let dest = self
            .alloc_register(self.closure_value_type_for_expr(&Expression::Function(func.clone())));
        self.emit(IrInstr::MakeClosure {
            dest: dest.clone(),
            func: func_id,
    ...
    pub(super) fn lower_arrow_with_preassigned_id(
        ...
        if let ast::ArrowBody::Block(block) = &arrow.body {
            let mut closure_locals = FxHashSet::default();
            for param in &arrow.params {
                super::collect_pattern_names(&param.pattern, &mut closure_locals);
            }
        ...
        match &arrow.body {
            ...
            ast::ArrowBody::Block(block) => {
                self.emit_js_function_var_hoists(&block.statements);
        ...
        let closure_ty = self.closure_value_type_for_expr(&Expression::Arrow(arrow.clone()));
    ...
    fn lower_typeof(&mut self, typeof_expr: &ast::TypeofExpression) -> Register {
        ...
        if let Expression::Identifier(ident) = argument {
                ...
                && self.shared_script_binding_slot(ident.name).is_none()
            {
                ...
                self.emit(IrInstr::Assign {
                    dest: dest.clone(),
                    value: IrValue::Constant(IrConstant::String("function".to_string())),
                });
                return dest;
            }
            if self.function_map.contains_key(&ident.name)
                && self
                    .resolve_identifier_binding(ident.name, ident.span.start)
                    .is_none()
            {
        ...
        if let Expression::Identifier(ident) = argument {
            ...
            let needs_non_throwing_lookup = needs_non_throwing_lookup
                && !self.semantic_identifier_is_builtin_global(ident.span.start)
    ...
    fn lower_new(&mut self, new_expr: &ast::NewExpression) -> Register {
        ...
        let dispatch_kind = dispatch
            .as_ref()
            .map(|dispatch| dispatch.kind)
            .unwrap_or(crate::semantics::ConstructorDispatchKind::DynamicConstructor);
        let semantic_callee_nominal_type_id = semantic_constructor
            .and_then(|target| target.callee_type_id)
            .and_then(|ty| self.nominal_type_id_from_type_id(ty));
        let result_ty = dispatch
            .as_ref()
            .and_then(|dispatch| dispatch.result_type_id)
            .filter(|ty| ty.as_u32() != UNRESOLVED_TYPE_ID)
            .unwrap_or(UNRESOLVED);
        let dest = self.alloc_register(result_ty);
        ...
        if new_expr.has_spread_arguments() {
            let ctor_value = match dispatch_kind {
                crate::semantics::ConstructorDispatchKind::NominalClass => {
                    if let Some(nominal_type_id) = semantic_callee_nominal_type_id
                        .or_else(|| self.infer_nominal_type_id(&new_expr.callee))
                    {
                        self.load_class_value_for_nominal_type(nominal_type_id)
                    } else {
        ...
        if matches!(
            ...
            let nominal_type_id = semantic_callee_nominal_type_id
                .or_else(|| self.infer_nominal_type_id(&new_expr.callee))
                .or_else(|| {
            ...
            if let Some(nominal_type_id) = nominal_type_id {
                    ...
                    .and_then(|info| info.constructor)
                {
                    ...
                    for (index, param_info) in ctor_params.iter().enumerate() {
                        if index >= provided_count {
                            if let Some(default_expr) = &param_info.default_value {
    ...
    fn lower_await(
        ...
        let checker_needs_waitall = matches!(
            ...
            Some(Awaitability::Definite | Awaitability::Maybe)
        );
        let lowered_needs_waitall = matches!(
            lowered_array_awaitability,
            Some(Awaitability::Definite | Awaitability::Maybe)
        );

        // Use WaitAll whenever the array could contain awaitables.
        // Runtime WaitAll already handles mixed arrays by treating non-task
        // elements as already-resolved literals.
        if checker_needs_waitall || lowered_needs_waitall {
    ...
    fn lower_instanceof(&mut self, instanceof: &ast::InstanceOfExpression) -> Register {
        ...
        let mut target_ty =
            self.resolve_structural_slot_type_from_annotation(&instanceof.type_name);
    ...
    fn resolve_class_from_type(&self, type_ann: &ast::TypeAnnotation) -> Option<NominalTypeId> {
        ...
        let resolved_ty = self.resolve_type_annotation(type_ann);
        if resolved_ty != UNRESOLVED {
            let mut visited = FxHashSet::default();
            if let Some(class_ty) =
                self.resolve_concrete_class_type_for_runtime_slots(resolved_ty, &mut visited)
            {
                if let Some(class_id) = self.nominal_type_id_from_type_id(class_ty) {
                    return Some(class_id);
                }
            }
            if let Some(class_id) = self.nominal_type_id_from_type_id(resolved_ty) {
                return Some(class_id);
            }
        }
    ...
    fn resolve_nullable_class_from_union(
        &self,
        type_ann: &ast::TypeAnnotation,
    ) -> Option<NominalTypeId> {
        use crate::parser::ast::types::{PrimitiveType, Type};
    ...
    fn first_callable_return_type(&self, ty_id: TypeId) -> Option<TypeId> {
        ...
        match self.type_ctx.get(ty_id) {
            ...
            Some(Type::Object(obj)) => {
                obj.call_signatures
                    .iter()
                    .find_map(|sig_ty| match self.type_ctx.get(*sig_ty) {
                        Some(Type::Function(func)) => Some(func.return_type),
                        _ => None,
                    })
            }
    ...
    pub(crate) fn type_has_construct_signature(&self, ty_id: TypeId) -> bool {
        ...
        match self.type_ctx.get(ty_id) {
            ...
            Some(Type::Function(_)) => self.js_this_binding_compat,
    ...
    fn first_construct_signature_return_type(&self, ty_id: TypeId) -> Option<TypeId> {
        ...
        match self.type_ctx.get(ty_id) {
            ...
            Some(Type::Function(func)) => Some(if self.js_this_binding_compat {
                UNRESOLVED
            } else {
                func.return_type
            }),
    ...
    fn type_is_runtime_dynamic_dispatch(&self, ty_id: TypeId) -> bool {
        ...
        match ty_id.as_u32() {
            UNRESOLVED_TYPE_ID | UNKNOWN_TYPE_ID | JSON_OBJECT_TYPE_ID => true,
            _ => self.type_ctx.get(ty_id).is_some_and(|ty| {
                matches!(ty, Type::Any | Type::Unknown | Type::Never | Type::JSObject)
            }),
        }
    }
    ...
    pub(super) fn infer_nominal_type_id(&self, expr: &Expression) -> Option<NominalTypeId> {
        if let Some(shape) = self.semantic_object_shape(expr.span().start, expr.span().end) {
            match shape.kind {
        ...
        match expr {
            ...
            Expression::TypeCast(cast) => {
                if self
                    .semantic_object_shape_kind(expr.span().start, expr.span().end)
    ...
    fn type_id_returns_task_like(&self, ty_id: TypeId) -> bool {
        ...
        match self.type_ctx.get(ty_id) {
            ...
            Some(Type::Reference(type_ref)) => self
                .type_ctx
                .lookup_named_type(&type_ref.name)
                .is_some_and(|named| self.type_id_returns_task_like(named)),
            Some(Type::TypeVar(tv)) => tv
                .constraint
    ...
    fn convert_binary_op(&self, op: &ast::BinaryOperator) -> BinaryOp {
        match op {
            ...
            ast::BinaryOperator::StrictEqual => BinaryOp::StrictEqual,
            ast::BinaryOperator::NotEqual => BinaryOp::NotEqual,
            ast::BinaryOperator::StrictNotEqual => BinaryOp::StrictNotEqual,
