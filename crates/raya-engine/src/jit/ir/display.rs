    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            JitInstr::StrictEq { dest, left, right } => {
                write!(f, "{} = strict_eq {}, {}", dest, left, right)
            }
            JitInstr::StrictNe { dest, left, right } => {
                write!(f, "{} = strict_ne {}, {}", dest, left, right)
            }
            ...
            JitInstr::StoreStatic { index, value } => {
                write!(f, "store.static {}, {}", index, value)
            }
            ...
            JitInstr::TaskThen {
                task,
                callback_index,
            } => write!(f, "task.then {}, @{}", task, callback_index),
            ...
            // Module
            JitInstr::LoadModule { dest, module_index } => {
                write!(f, "{} = load.module @{}", dest, module_index)
            }
            JitInstr::LoadConst { dest, const_index } => {
                write!(f, "{} = load.const @{}", dest, const_index)
            }
