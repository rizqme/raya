...
**Crates:**
- `raya-core` - Module loader and runtime linking
- `raya-pm` - Package manager and cache

**Scope:** This milestone focuses on **VM-side module system only** (loading, linking, caching). Compilation aspects (parsing .raya, code generation) are handled separately in compiler milestones.
...
**Tasks:**
- [ ] **Module Bytecode Format** (`raya-bytecode`)
...
- [ ] **Module Loader** (`raya-core/module.rs`)
...
- [ ] **Module Registry** (`raya-core/registry.rs`)
...
**Tasks:**
- [ ] **Import Resolver** (`raya-core/import.rs`)
  ...
  - [ ] Resolve local file imports
    ```rust
    fn resolve_local(path: &str, current: &Path) -> Result<PathBuf>
    ```
  - [ ] Resolve package imports (check cache first)
    ```rust
    fn resolve_package(spec: &str, cache: &Cache) -> Result<PathBuf>
    ```
  - [ ] Resolve URL imports (fetch if not cached)
    ```rust
    fn resolve_url(url: &str, cache: &Cache) -> Result<PathBuf>
    ```
  - [ ] Import path normalization

- [ ] **Dependency Graph** (`raya-core/deps.rs`)
  ...
  - [ ] Circular dependency detection
    ...
    ```
  - [ ] Error reporting for circular deps
...
**Tasks:**
- [ ] **Cache Directory Structure** (`raya-pm/cache.rs`)
  - [ ] Initialize cache directory on first run
    ...
    └── config.toml
    ```
  - [ ] Cache directory permissions (platform-specific)
  - [ ] Cache version migration support

- [ ] **Content-Addressable Storage** (`raya-pm/storage.rs`)
  - [ ] Store modules by SHA-256 hash
    ```rust
    fn store(module: &[u8]) -> Result<[u8; 32]> {
...
- [ ] **Cache Eviction** (`raya-pm/eviction.rs`)
...
**Tasks:**
- [ ] **Symbol Resolution** (`raya-core/linker.rs`)
  - [ ] Resolve imported symbols from exports
    ```rust
    fn link_import(import: &Import, module: &Module) -> Result<Value> {
...
- [ ] **Module Initialization** (`raya-core/init.rs`)
...
- [ ] **Module Namespacing** (`raya-core/namespace.rs`)
  - [ ] Create isolated namespace per module
  - [ ] Prevent global scope pollution
  - [ ] Support for multiple major versions of same package
    ```rust
    struct Namespace {
        modules: HashMap<String, HashMap<u32, Arc<Module>>>, // name -> major_version -> module
    }
    ```
...
**Tasks:**
- [ ] **raya.toml Parser** (`raya-pm/manifest.rs`)
...
- [ ] **Lockfile Management** (`raya-pm/lockfile.rs`)
...
- [ ] **Metadata Format** (`raya-pm/metadata.rs`)
...
**Goal:** Resolve package versions according to semver constraints (runtime logic).

**Tasks:**
- [ ] **Semver Parser** (`raya-pm/semver.rs`)
  - [ ] Parse version strings (`1.2.3`, `^1.2.0`, `~1.5.0`)
  - [ ] Implement version constraint matching
    ```rust
    fn matches(constraint: &Constraint, version: &Version) -> bool
    ```
  - [ ] Version ordering and comparison

- [ ] **Dependency Resolver** (`raya-pm/resolver.rs`)
...
- [ ] **Conflict Resolution** (`raya-pm/conflict.rs`)
  ...
  - [ ] Conflict resolution strategies (relaxed vs strict)
...
**Tasks:**
- [ ] **Path Resolution** (`raya-pm/path.rs`)
  - [ ] Resolve relative paths from manifest
    ```rust
    fn resolve_path_dep(path: &str, project_root: &Path) -> Result<PathBuf>
    ```
  - [ ] Validate path dependency has raya.toml
  - [ ] Watch for changes in path dependencies (hot reload)
  - [ ] Path normalization (cross-platform)

- [ ] **Local Module Loading** (`raya-core/local.rs`)
...
1. Module bytecode format & loader (Phase 1)
2. Import resolution (Phase 2)
3. Global cache (Phase 3)
4. Module linking (Phase 4)
5. Package metadata (Phase 5)
6. Semver & resolution (Phase 6)
7. Local paths (Phase 7)
8. Testing (Phase 8)

**Reference:** `design/MODULES.md` (Complete specification)

**Note:** Package manager CLI (`raya install`, `raya add`, etc.) and registry client (HTTP downloads) will be implemented in a separate milestone after core module system is working.
